...
In Section~\ref{sec:differential_privacy}, we cover some necessary background for understanding the basics of differential privacy.
Section~\ref{sec:dependent_types_in_idris} serves as a brief tutorial to Idris with a focus on the language features we make use of in our implementation.
Section~\ref{sec:function_sensitivity} combines the contents of the previous two sections in an example implementation of function sensitivity.
Section~\ref{sec:typing_the_relational_algebra} reviews the relational algebra and how we represent it using dependent types.
Section~\ref{sec:our_implementation} covers the differential privacy layer that we built over top of the fully-typed relational algebra.
We evaluate our work in Section~\ref{sec:evaluation} and provide concluding remarks in Section~\ref{sec:conclusion}.
