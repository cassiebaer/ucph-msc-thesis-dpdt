...
The intrinsic randomness of the function prevents an attacker from deducing anything about the input.
Now consider a randomized query, or mechanism, $\mathcal M(D) : \mathbb D \rightarrow \mathbb R$.
We say that such a query is $\epsilon$-differentially-private if the inclusion of your data affects your privacy risk by at most $e^\epsilon$. % TODO: What is 'privacy risk'

\begin{defn}\label{def:diffpriv}
  A randomized mechanism, $\mathcal{M}(D) : \mathbb D \rightarrow \mathbb R$, is $\epsilon$-differentially-private if, for all databases $D_1 \sim D_2$, and all $S \in \mathbb R$,
...
\begin{enumerate}
	...
	\item Repeat step 1 and 2 \texttt{X} number of times
\end{enumerate}
...
Currently, this backend compiles queries to a single SQLite queries.
