...
In a paper from 2008, Ganta et al. show that \textit{k-anonymity} and other proposed partition-based anonymization approaches are vulnerable to composition attacks\cite{ganta2008composition}.
...
An attacker can with this technique extract more information about individuals in the anonymized set.
...
% TODO 1 : this section overlaps with chapter on function sensitivity (Which chapter?)
...
There are potentially many randomized mechanisms that can be used to accomplish indistinguishability between neighbouring databases.
Two well known randomized mechanisms used in differential privacy literature are Laplace- and Gaussian distributions.
A Laplacian noise distribution with a mean of zero and a scale parameter of $fn-sensitivity/\epsilon$ guarantees $\epsilon$-differential privacy.
A Gaussian noise distribution can provide a slightly more relaxed differential privacy guarantee, namely ($\epsilon$, $\delta$)-differential privacy, where delta is an additive on the privacy bound. % TODO: Provide cite
\begin{defn}\label{def:diffpriv_delta}
A randomized mechanism, $\mathcal{M}(D) : \mathbb D \rightarrow A$, is ($\epsilon$, $\delta$)-differentially-private if, for all databases $D_1 \sim D_2$, and all $S \in A$,
  $$Pr[\mathcal M(D_1)\in S] \le e^\epsilon \times Pr[\mathcal M(D_2)\in S] + \delta  $$
\end{defn}

We will however ignore Definition \ref{def:diffpriv_delta} in this report, as we will be using Laplace noise to achieve $\epsilon$-differential privacy in our language.
...
Custom syntax rules allow embedded language develoeprs to extend the parser with EDSL-specific syntax constructs.
...
Poorly implemented syntax rules can cause the parser to diverge.

\section{Conclusion}\label{sec:conclusion}

We have demonstrated that a differentially private query languge can be represented in a dependent type system, where the privacy guarantees are enforced statically in types.
A prototype named DPDT is implemented in Idris, providing basic transformation and aggregation operations.
DPDT guarantees differential privacy statically, which means that any valid program written in it is differentially private.
