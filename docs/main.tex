...
\begin{lstlisting}[float,caption={Aggregation result ADT},label={lst:DPQ_private}]
...
DPQ is useful for simple statistical work that can be represented in one aggregation query, but for more complex algorithms this is not sufficient.
DPDT is a more flexible language, in that it supports sequencing of queries, expanding the algorithm space to e.g. several machine learning algorithms.

DPQ and DPDT shares much of the implementation as it uses mostly the same abstract types.
The difference is in aggregation return types.

If you recall the \texttt{Private} ADT from Section \ref{seq:DPQ_aggregations} it was just a string with an associated sensititivity.
DPDT has its own implementation of \texttt{Private}, shown in Listing \ref{lst:mechanisms}.
It still acts as the result type of aggregation functions, but has some extended functionality.
It is parameterized by the aggregation function's return type, instead of just \texttt{String} as in DPQ.
\texttt{CrapGen} is a fairly good random number generator, as the name implies, and is used in the process of adding noise.
...
In addition, DPDT has functions for handling the sequencing of multiple \texttt{Private} results.
...
The aggregation functions in DPDT look very similar to the ones in DPQ.
Listing \ref{lst:DPDT_aggregations} shows DPDT's \texttt{noisyCount}.
We can see that the return type includes what type the aggregation returns (here, and for most differentially private aggregations, a \texttt{Double}).
The sensitivity calculations work the same as in DPQ.

\begin{lstlisting}[float,caption={Representing aggregations in DPDT},label={lst:DPDT_aggregations}]
...
noisyCount : (q:Query ListRow s c) -> (e:Epsilon) -> Private (c*e) Double
...
Another flexibility improvement implemented for our Idris backend of DPDT is the \texttt{PureFn} expression in Listing \ref{lst:purefn}.

\begin{lstlisting}[float,caption={PureFn expression constructor}, label={lst:purefn}]
PureFn : (a -> b) -> Expr s a -> Expr s b
\end{lstlisting}

\texttt{PureFn} is for lifting an Idris function into an expression, allowing for a lot of flexibility for the backends that can convert the function to the target language.
It is actually a part of RADT's expression language, which means it is a part of DPQ as well.
However, it is not trivial to convert an arbitrary Idris function to a query language expression (e.g. SQLite).
Therefore, use of this functionality with the SQLite backend will just return a friendly error message.
Luckily it is fairly easy to run an Idris function within Idris, so the DPDT Idris backend has this feature readily available.


As described in Section \ref{sec:pinq}, in PINQ the user defines a global privacy budget for a differentially private program.
As queries are run, the cost of the queries are subtracted from the budget, and when the budget is spent no more data access is allowed.
We have a slightly different approach.
In DPDT, we have no global budget, but provides the user with the total cost of the private computation in the type.
